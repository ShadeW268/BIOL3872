...
#loop solution
#for (g in groundfishes$num_cpue) {
  #groundfish_total_abundance <- sum(groundfishes$num_cpue)
...
for(sp in species){
 for(yr in year){
    ...
    #add to results
    abundance_totals <- rbind(abundance_totals,
                              data.frame(species=sp, year=yr, total_cpue=total))
 }
}

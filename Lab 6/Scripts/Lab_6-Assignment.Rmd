...
datetime1 <- as.POSIXct("February-28-2010 2:38PM", format = "%B-%d-%Y %I:%M%p", tz ="Canada/Atlantic")
...
datetime2 <- as.POSIXct("Sep/14/2015 09:50:45", format = "%b/%d/%Y %I:%M:%S", tz = "Canada/Atlantic")
...
datetime3 <- as.POSIXct("04-08-2024 16:39", format = "%m-%d-%Y %H:%M", tz = "Canada/Atlantic")
```
:::
...
myear_median_tk = Bedford_ts %>%
  summarize_by_time(med_richness = median(Richness, na.rm = TRUE),
                    .date_var = Date,
                    .by = 'month')
myear_median_tk$month <- month(myear_median_tk$Date)
myear_median_tk$year <- year(myear_median_tk$Date)
myear_median_tk$month_full <- month.name[myear_median_tk$month]
myear_median_tk$month_full <- factor(myear_median_tk$month_full, levels = month.name)                    
monthly_richness <- myear_median_tk %>%
  ggplot(aes(x = month_full, y = med_richness, color = factor(year)))+
  geom_point()+
  theme_classic(12)+
...
weekly_median <- tapply(Bedford_ts$Richness, Bedford_ts$week, median)

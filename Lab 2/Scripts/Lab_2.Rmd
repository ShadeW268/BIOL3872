...
```{r}
# Your code here:
fav_animal <- "Mobula birostris"  # species name of my favourite animal
average_lifespan_yrs <- 50   # average lifespan of species
is_extant <- TRUE   # TRUE (any living species is TRUE)
class(fav_animal)  # data type of my variable
class(average_lifespan_yrs)
class(is_extant)
as.character(average_lifespan_yrs)  # converts variable value from numeric to character
is.character(average_lifespan_yrs)  # checks if the variable is a character (FALSE because unassigned converted variable)
average_lifespan_yrs <- as.character(average_lifespan_yrs)  # assigns average lifespan variable the converted characted value
is.character(average_lifespan_yrs) # TRUE (variable is now assigned the correct converted value)
```
